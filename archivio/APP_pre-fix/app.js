  ...
  function createMultiSelectDropdown(containerId, options, filterKey, placeholder, allLabel) {
    ...
    function positionMenu() {
      ...
      // Stima l'altezza del menu (usa un valore ragionevole se non è ancora renderizzato)
      const estimatedMenuHeight = 300;
      const spaceBelow = viewportHeight - triggerRect.bottom;
      const spaceAbove = triggerRect.top;
      
      // Apri sopra solo se c'è poco spazio sotto E più spazio sopra
      const openAbove = spaceBelow < estimatedMenuHeight && spaceAbove > spaceBelow && spaceAbove > estimatedMenuHeight;
      
      // Calcola la posizione verticale
      let topPosition;
      if (openAbove) {
        // Apri sopra: posizione = top del trigger - altezza menu - gap
        // Usa l'altezza effettiva se disponibile, altrimenti stima
        const actualHeight = menu.offsetHeight || estimatedMenuHeight;
        topPosition = triggerRect.top - actualHeight - 4;
        // Assicurati che non vada sopra lo schermo
        if (topPosition < 8) {
          topPosition = 8;
        }
      } else {
        // Apri sotto: posizione = bottom del trigger + gap
        topPosition = triggerRect.bottom + 4;
        // Assicurati che non vada sotto lo schermo
        const maxTop = viewportHeight - (menu.offsetHeight || estimatedMenuHeight) - 8;
      ...
      // Ricalcola dopo che il menu è renderizzato per posizionamento preciso
      requestAnimationFrame(() => {
        const actualHeight = menu.offsetHeight;
        if (openAbove && actualHeight) {
          // Ricalcola la posizione sopra con l'altezza reale
          const newTop = triggerRect.top - actualHeight - 4;
          if (newTop >= 8) {
            menu.style.top = `${newTop}px`;
          }
        } else if (!openAbove && actualHeight) {
          // Assicura che non vada sotto lo schermo con l'altezza reale
          const maxTop = viewportHeight - actualHeight - 8;
          const currentTop = parseFloat(menu.style.top) || topPosition;
